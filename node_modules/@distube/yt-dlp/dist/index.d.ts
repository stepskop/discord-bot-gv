...
declare type YtDlpResponse = YtDlpVideo | YtDlpPlaylist;
interface YtDlpOptions {
    update?: boolean;
}

declare const raw: (url: string, flags?: YtDlpFlags, options?: execa.Options<string>) => execa.ExecaChildProcess<string>;
declare const json: (url: string, flags?: YtDlpFlags, options?: execa.Options<string>) => Promise<YtDlpResponse>;
declare const download: () => Promise<string>;

declare class YtDlpPlugin extends ExtractorPlugin {
    constructor({ update }: YtDlpOptions);
    validate(): boolean;
    resolve<T>(url: string, { member, metadata }: {
        member?: GuildMember;
        metadata?: T;
    }): Promise<Playlist<T> | Song<T>>;
    getStreamURL(url: string): Promise<string>;
}

export { YtDlpPlugin, download, json, raw };
