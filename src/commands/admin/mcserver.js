            ...
            case 'start':
                if (interaction.member.id === config.admin) {
                  try {
                        ...
                        interaction.reply({embeds: [new EmbedBuilder().setTitle('Server starting!').setColor(Colors.DarkGrey).setDescription("<t:"+Math.floor(new Date().getTime() / 1000)+ ":d> at <t:"+ Math.floor(new Date().getTime() / 1000)+ ":t>")], ephemeral: true})  
                    } catch (error) {
                        interaction.reply({embeds:[new EmbedBuilder().setColor(Colors.Red).setDescription('Something went wrong: '+ error)]})
                    }   
                } else {
                    interaction.reply({embeds: [new EmbedBuilder().setDescription("You are not permitted to start our Minecraft server")], ephemeral: true})
                }
                break;
                ...
                try {
                    ...
                    .then((res) => {
                        if (res.status != 200) {
                            ...
                            console.log(res.data)
                            interaction.reply({embeds: [new EmbedBuilder().setTitle('Server is offline').setDescription('...or i am broken')], ephemeral: true})
                        }
                        else if (Array.isArray(res.data.tunnels) && res.data.tunnels.length) {  
                            const fullIp = res.data.tunnels[0].public_url.slice(6)
                            interaction.reply({embeds: [new EmbedBuilder().setColor(Colors.DarkGreen).setTitle('<:mc:971451702341414953> Server is on!').setDescription('\nIP is:\n**'+fullIp+'**\n\n' + "<t:"+ Math.floor(new Date().getTime() / 1000)+ ":d> at <t:"+ Math.floor(new Date().getTime() / 1000)+ ":t>")], ephemeral: true})
                        } else {
                            interaction.reply({embeds: [new EmbedBuilder().setTitle('Server is offline').setDescription('...or i am broken')], ephemeral: true})
                        }
                    })
