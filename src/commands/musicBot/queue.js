...
module.exports = (config, client, interaction, voiceChannel, queue) => {
    try {
        if (!queue) {
            return interaction.reply({embeds: [new EmbedBuilder().setDescription("There is no \*\*queue\*\*")]})
        }
        else {
            try {
                ...
                if (Object.keys(queue.songs).length < 20) {
                    return interaction.reply({embeds: [new EmbedBuilder().setColor(Colors.DarkPurple).setDescription(`${queue.songs.map(
                        (song, id) => `\n**${id + 1}**. ${song.name} - \`${song.formattedDuration}\``)}`
                        )]})
                } else {
                    return interaction.reply({embeds: [new EmbedBuilder().setColor(Colors.DarkPurple).setDescription('In queue is ' + Object.keys(queue.songs).length + " songs")]})
                }
